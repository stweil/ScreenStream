...
internal open class VncEvent(val priority: Int) {
    ...
    internal sealed class Intentable(priority: Int) : VncEvent(priority), Parcelable {
        ...
        internal fun toIntent(context: Context): Intent = VncModuleService.getIntent(context).putExtra(EXTRA_PARCELABLE, this)
    }

    internal data class CastPermissionsDenied(val startAttemptId: String) : VncEvent(Priority.RECOVER_IGNORE)
    ...
    ) : VncEvent(Priority.START_PROJECTION)
}
